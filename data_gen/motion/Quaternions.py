...
class Quaternions:
    ...
    that allows it to act as if it were an ndarray of
    ...
    def __init__(self, qs):
        ...
        raise TypeError('Quaternions must be constructed from iterable, numpy array, or Quaternions, '
                        'not %s' % type(qs))
    ...
    """ Adding Quaternions is just Defined as Multiplication """
    ...
    def __mul__(self, other):
        ...
        When multiplying a Quaternions array by a scalar
    ...
    def __div__(self, other):
        ...
        as multiplication of one over the supplied value.
    ...
    def log(self):
        ...
        print('self', self)
        print('self.normalized()', self.normalized())
        print('norm', norm)
        print('imgs', imgs)
        print('lens', lens)
    ...
    def average(self):
        ...
        if len(self.shape) == 1:
            ...
            raise NotImplementedError('Cannot average multi-dimensional Quaternions')
    ...
    def slerp(cls, q0s, q1s, a):
        ...
        linear = (1.0 - length) < 0.01
        omegas = np.arccos(length[~linear])
    ...
    @classmethod
    def look_rotation(cls, up, forward):
        up = utils.normalize_vec(up)
        forward = utils.normalize_vec(forward)
        right = np.cross(up, forward)
        transforms = np.stack([right, up, forward], axis=-1)

        return cls.from_transforms(transforms)
