...
def transforms_global(anim):
    ...
    anim_locals = transforms_local(anim)
    anim_globals = transforms_blank(anim)

    anim_globals[:, 0] = anim_locals[:, 0]
    
    for i in range(1, anim.shape[1]):
        anim_globals[:, i] = transforms_multiply(anim_globals[:, anim.parents[i]], anim_locals[:, i])
